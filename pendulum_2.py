...
class Pendulum:
    ...
    * visuals: the list of all the 'visual' 3D objects to render the robot, each element of the list being
    ...
    def __init__(self, nbJoint=1, noise_stddev=0.0, ):
        ...
        self.withSinCos = True  # If true, state is [cos(q),sin(q),qdot], else [q,qdot]
...
#    def nobs(self): return self.nx+self.withSinCos
    ...
    def obs(self, x):
        ...
        if self.withSinCos:
            return np.vstack( ([np.cos(x[0][qi]),np.sin(x[0][qi]), x[1][qi]] )for qi in range(0, len(x[0]) ) )
        else: return x.copy()
    ...
    def dynamics(self, x, u, display=False):
        ...
        x[0] = modulePi(q)
        #x[1][0] = np.clip(v[0], -self.vmax1, self.vmax1)
        #x[1][1] = np.clip(v[1], -self.vmax2, self.vmax2)
